...
const search = async () => {
  const result = await queryPageApi(
    searchClazz.value.name, searchClazz.value.begin,
...
const save = async () => {
  ...
  clazzFormRef.value.validate(async (valid) => {
    ...
    const result = clazz.value.id ? await updateApi(clazz.value) : await addApi(clazz.value)
    if (result.code) {
...
const rules = ref({
  ...
  beginDate: [{ required: true, message: '开课时间是必填项', trigger: 'change' }],
      ...
      <el-row :gutter="20">
        <el-col :span="12">
          <el-form-item label="开课时间" prop="beginDate">
            <el-date-picker v-model="clazz.beginDate" type="date" style="width: 100%"
              placeholder="选择开课时间" format="YYYY-MM-DD" value-format="YYYY-MM-DD" />
          </el-form-item>
        </el-col>
        <el-col :span="12">
          <el-form-item label="结课时间" prop="endDate">
            <el-date-picker v-model="clazz.endDate" type="date" style="width: 100%"
              placeholder="选择结课时间" format="YYYY-MM-DD" value-format="YYYY-MM-DD" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
