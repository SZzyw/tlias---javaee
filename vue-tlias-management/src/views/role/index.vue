...
const save = async () => {
  const result = role.value.id
    ? await updateApi(role.value)
    : await addApi(role.value);
  if (result.code) {
...
const savePermissions = async () => {
  const result = await saveRolePermissionsApi(
    currentRoleId.value,
    ...
    <section class="page-header">
      <div class="page-header-copy">
        ...
        <p class="page-description">
          按职责维护角色、权限与人员绑定范围，确保后台访问能力与组织分工一致。
        </p>
      </div>
  ...
  <el-dialog v-model="permissionDialogVisible" title="分配权限" width="700">
    <div class="permission-groups">
      <div
        ...
        v-for="(groupItems, groupName) in permissionGroups"
        :key="groupName"
      >
