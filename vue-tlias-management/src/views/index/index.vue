...
import { Avatar, Delete, HelpFilled, HomeFilled, UserFilled, WarningFilled } from '@element-plus/icons-vue'
...
const statCards = computed(() => [
  { label: '员工总数', value: dashboard.value.empCount || 0, icon: Avatar, theme: 'copper', note: '组织成员总览' },
  { label: '学员总数', value: dashboard.value.stuCount || 0, icon: UserFilled, theme: 'forest', note: '当前学员规模' },
  { label: '班级总数', value: dashboard.value.clazzCount || 0, icon: HomeFilled, theme: 'amber', note: '班级资源分布' },
  { label: '部门总数', value: dashboard.value.deptCount || 0, icon: HelpFilled, theme: 'rose', note: '组织结构节点' },
  { label: '违纪总次数', value: dashboard.value.totalViolation || 0, icon: WarningFilled, theme: 'slate', note: '纪律事件累计' },
  { label: '违纪总扣分', value: dashboard.value.totalViolationScore || 0, icon: Delete, theme: 'plum', note: '行为扣分统计' }
])
...
const initCharts = () => {
  if (empTrend.value.length > 0) {
    ...
    empChartInstance.setOption({
      ...
      yAxis: { type: 'value' },
      series: [{ type: 'line', data: empTrend.value.map((d) => d.value), smooth: true, areaStyle: {}, itemStyle: { color: '#8f623b' } }]
    })
  }
  ...
  if (stuTrend.value.length > 0) {
    ...
    stuChartInstance.setOption({
      ...
      yAxis: { type: 'value' },
      series: [{ type: 'line', data: stuTrend.value.map((d) => d.value), smooth: true, areaStyle: {}, itemStyle: { color: '#4f7f73' } }]
    })
  }
  ...
  if (violations.value.length > 0) {
    ...
    violationChartInstance.setOption({
      ...
      yAxis: { type: 'value' },
      grid: { bottom: 60 },
      series: [{ type: 'bar', data: violations.value.map((d) => d.score), itemStyle: { color: '#c86d5f' } }]
    })
  }
}
  ...
  <div class="page-shell dashboard-page" v-loading="loading">
    <section class="page-header dashboard-hero">
      ...
      <div class="page-actions dashboard-actions">
        ...
        <span class="page-chip" v-if="topViolation">重点关注 {{ topViolation.name }}</span>
      </div>
    </section>

    <section class="stats-grid">
      <article v-for="card in statCards" :key="card.label" class="stat-panel" :class="`theme-${card.theme}`">
        <div class="stat-icon">
      ...
      <div class="page-panel">
        <div class="page-section-header">
          <div>
            ...
            <p class="page-section-subtitle">观察组织规模变化，辅助招聘与排班决策。</p>
          </div>
        </div>
      ...
      <div class="page-panel">
        <div class="page-section-header">
          <div>
            ...
            <p class="page-section-subtitle">查看班级扩张节奏，为教学资源投入提供依据。</p>
          </div>
        </div>
      ...
      <div class="page-panel">
        <div class="page-section-header">
          <div>
            ...
            <p class="page-section-subtitle">识别风险学员与重点班级，方便进一步跟进。</p>
          </div>
        </div>
      ...
      <div class="page-table-card">
        <div class="table-panel-header">
          <div>
            ...
            <p class="page-section-subtitle">结合次数与扣分，快速锁定需要干预的对象。</p>
          </div>
        </div>
        <el-table :data="violations" border style="width: 100%" max-height="360" size="small">
